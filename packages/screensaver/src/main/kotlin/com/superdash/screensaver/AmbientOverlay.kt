...
fun AmbientOverlay(
    ...
    Box(modifier.fillMaxSize()) {
        ...
        var eventLabel by remember { mutableStateOf(calendarLabelOrNull(calendar)) }
        LaunchedEffect(calendar) {
            eventLabel = calendarLabelOrNull(calendar)
            while (true) {
                now = formatTime()
                date = formatDate()
                eventLabel = calendarLabelOrNull(calendar)
...
private fun calendarLabelOrNull(snapshot: CalendarSnapshot?): String? {
    if (snapshot == null) {
        return null
    }
    return formatCalendarLabel(snapshot, Clock.System.now(), TimeZone.currentSystemDefault())
}

private fun formatTime(): String = SimpleDateFormat("HH:mm", Locale.getDefault()).format(Date())

private fun formatDate(): String = SimpleDateFormat("EEEE, d MMMM", Locale.getDefault()).format(Date())
