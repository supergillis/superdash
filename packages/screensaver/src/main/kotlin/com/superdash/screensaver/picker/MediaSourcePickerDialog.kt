...
import com.superdash.screensaver.R
...
fun MediaSourcePickerDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text(
                text = current?.node?.title ?: stringResource(R.string.screensaver_picker_title_default),
                ...
                when {
                    ...
                    error != null -> {
                        Column(
                            ...
                            Text(
                                stringResource(
                                    R.string.screensaver_picker_load_error,
                                    error ?: stringResource(R.string.screensaver_picker_browse_failed),
                                ),
                            )
                            Spacer(Modifier.size(8.dp))
                            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                                TextButton(onClick = { retryKey += 1 }) {
                                    Text(stringResource(R.string.screensaver_picker_retry))
                                }
                                if (backStack.isNotEmpty()) {
                                    TextButton(
                                        ...
                                        Text(stringResource(R.string.screensaver_picker_back))
                                    }
                                }
                            }
                        }
                    ...
                    current != null -> {
                        Column {
                            Text(
                                text =
                                    stringResource(
                                        R.string.screensaver_picker_current_label,
                                        currentFolderLabel(current!!.node, backStack),
                                    ),
                            ...
                            Text(
                                text =
                                    stringResource(
                                        R.string.screensaver_picker_selected_label,
                                        selected?.title
                                            ?: stringResource(R.string.screensaver_picker_selected_none),
                                    ),
                                modifier = Modifier.padding(bottom = 8.dp),
                            )
                            OutlinedTextField(
                                ...
                                label = { Text(stringResource(R.string.screensaver_picker_search_label)) },
                                singleLine = true,
                                modifier = Modifier.fillMaxWidth(),
                            )
                            Spacer(Modifier.size(8.dp))
                            val rootLabel = stringResource(R.string.screensaver_picker_root_label)
                            val backLabel =
                                if (backStack.isNotEmpty()) {
                                    backStack.dropLast(1).lastOrNull()?.title ?: rootLabel
                                } else {
                                    null
                                }
                            PickerList(
                            ...
                            if (current!!.children.isEmpty()) {
                                Spacer(Modifier.size(8.dp))
                                Text(stringResource(R.string.screensaver_picker_no_media))
                            }
                        }
                    }
                }
        ...
        confirmButton = {
            Button(
                ...
                Text(stringResource(R.string.screensaver_picker_use_folder))
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) {
                Text(stringResource(R.string.screensaver_picker_cancel))
            }
        },
    )
}
...
private fun PickerList(
    ...
    LazyColumn(
        ...
        if (backLabel != null) {
            item {
                Row(
                    ...
                    Spacer(Modifier.size(8.dp))
                    Text(stringResource(R.string.screensaver_picker_back_to, backLabel))
                }
            }
        }
        item {
            ListItem(
                headlineContent = { Text(stringResource(R.string.screensaver_picker_this_folder)) },
                supportingContent = {
                    if (canSelectCurrent) {
                        Text(currentNode.title)
                    } else {
                        Text(stringResource(R.string.screensaver_picker_no_images))
                    }
                },
                trailingContent =
                    if (selectedSource?.mediaContentId == currentNode.mediaContentId) {
                        {
                            Icon(
                                ...
                                contentDescription = stringResource(R.string.screensaver_picker_selected_description),
                            )
                        }
                    } else {
                        null
        ...
        if (children.isNotEmpty() && filteredChildren.isEmpty()) {
            item {
                ListItem(
                    headlineContent = { Text(stringResource(R.string.screensaver_picker_no_matching)) },
                    supportingContent = { Text(stringResource(R.string.screensaver_picker_try_different_search)) },
                )
            }
        }
